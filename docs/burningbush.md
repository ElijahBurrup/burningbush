...
- **Scratch offs:** `SCRATCH_LADDER`, in the order they are offered: **Library** (memorize your first verse) → **Bible** (finish one round of Practice Verses) → **Memory Palaces** (5 verses + a 5-answer streak) → **Bible Stories** (15 verses + 2 palaces). `pendingScratchRung()` returns the *lowest unwon rung whose gate passes*, so **array order is offer order** — moving a rung in the array moves it in the ladder. Claiming the Bible opens the book the earning verse came from. Winning it also runs `bibleTakesItsPlace()`:  the fourth slot empties, palace steps right, then verse, then learn, each into the slot the one before it left, and only then does the Bible drop in at the front.
- **The Library arrives whole, with two stickers.** A book lesson offers a verse from that book (`renderLessonDone` → `openSuggestedForBook`, which stays on the Learn screen while the Library is still locked), and that first verse wins the Library ticket. The hub then draws every tile; `Spaced Repetition` and `Practice Word for Word` sit under `foilHTML()` stickers naming what opens them — **Build a Memory Palace** (`.sl-sr`, lifted by `srUnlocked()`, which reads the *film*, not the palace count) and **Practice Verses to Unlock** (`.sl-w4w` on the hub, `.sl-lvw4w` on the verse screen, both lifted by `libUsed("verses")`). `Prog.libUsed` records which tools have actually been used; it lifts the Word for Word sticker and gates the Bible rung. There is no ladder of presents — that shipped in v1.101 and was replaced in v1.103.
...
- **Two different rules read like one: what the app asks of you, and what you ask of it.** A verse is held out of the *review* queue until its first look back four hours after learning (`firstReviewDone` in `verseDue`/`versesDueCount`). The same guard was on `deckKeys()`, the *free practice* deck — so a new reader with two verses got an empty deck and a "Deck cleared!" screen whose New Round button rebuilt it for ever. Free practice is something a reader asks for; it never waits. A verse cleared *before* its four-hour mark does return to the deck at the mark, which is why `deckKeys` compares `o.lc` against `firstReviewAt(o)`.
